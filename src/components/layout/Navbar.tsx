        ...
        <div className="flex items-center gap-[clamp(12px,2.2vw,30px)] whitespace-nowrap">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="no-underline font-medium text-[clamp(12px,1.4vw,15px)] text-[#1d1d1f] transition-colors duration-200 hover:text-[#1f4a37]"
            >
              {link.label}
            </a>
          ))}
          ...
          <a
            href="#contact"
            className="inline-flex items-center bg-[#1f4a37] text-white no-underline font-semibold text-[clamp(12px,1.4vw,15px)] px-[17px] py-2 rounded-full transition-colors duration-200 hover:bg-[#25593f]"
          >
            Réserver
          </a>
        </div>
